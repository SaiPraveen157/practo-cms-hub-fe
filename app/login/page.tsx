...
export default function LoginPage() {
  ...
  const handleGoogleCredential = useCallback(
    async (idToken: string) => {
      ...
      try {
        ...
        const roleHome = userData?.role
          ? getHomePathForRole(userData.role as UserRole)
          : "/"
  ...
  useEffect(() => {
    ...
    const roleHome = user?.role
      ? getHomePathForRole(user.role as UserRole)
      : "/"
    router.replace(roleHome)
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    id="email"
                    type="email"
                    placeholder="medical@practo.com"
                    value={email}
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <Input
                      ...
                      required
                      disabled={loading}
                      className="h-11 rounded-lg border-gray-300 bg-gray-50/80 px-4 pr-11 text-black focus:border-blue-500 focus:ring-blue-500/20"
                    />
                    <Button
                      type="button"
                      variant="ghost"
                      size="icon"
                      className="absolute top-1/2 right-1 size-9 -translate-y-1/2 text-gray-500 hover:text-gray-700"
                ...
                <Button
                  type="submit"
                  disabled={loading}
                  className="h-12 w-full cursor-pointer rounded-lg border-0 bg-linear-to-r from-[#518dcd] to-[#7ac0ca] text-base font-semibold text-white shadow-md transition-opacity hover:opacity-95"
                >
                  {loading ? (
                    <>
