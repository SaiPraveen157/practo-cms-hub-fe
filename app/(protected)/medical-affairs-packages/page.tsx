      ...
      <div className="mx-auto max-w-4xl space-y-6">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Phase 6 — Video track review (parallel with Brand metadata). Use{" "}
            <strong>History</strong> to see packages you approved or rejected.
          </p>
        </div>
            ...
            {queueFiltered.length === 0 ? (
              ...
              <ul className="space-y-3">
                {queuePageSlice.map((p) => (
                  <li key={p.id}>
                    <QueueRow pkg={p} href={`/medical-affairs-packages/${p.id}`} />
                  </li>
                ))}
              </ul>
            )}
            ...
            <ul className="space-y-3">
              {historyFiltered.map((p) => (
                <li key={p.id}>
                  <QueueRow
                    pkg={p}
                    href={`/medical-affairs-packages/${p.id}`}
        ...
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="text-xs text-muted-foreground">
              Video {pkg.videoTrackStatus} · Metadata {pkg.metadataTrackStatus}{" "}
              · v{pkg.version}
            </span>
          </div>
          <p className="mt-1 font-medium">{pkg.title}</p>
          <p className="text-sm text-muted-foreground">
            {pkg.script?.title ?? "Script"} · {formatPackageDate(pkg.updatedAt)}
          </p>
