...
import type { LanguageVideoAsset } from "@/types/language-package"
import type { VideoComment } from "@/types/video"
import { formatVideoTimestamp } from "@/lib/video-timestamp"
import { Loader2, XCircle } from "lucide-react"

export type LangRejectFieldState = { flag: boolean; comment: string }

/** Same shape as Phase 6 metadata reject thumbnails (`MetaRejectThumbState`). */
export type LangRejectThumbRow = { reject: boolean; comment: string }

export type LangRejectDraft = {
...
export function emptyLangRejectDraft(
...
export function RejectLanguageVideoDialogBody({
  asset,
  videoLabel,
  draft,
  setDraft,
  ...
  const intro =
    variant === "approver" ? (
      <>
        ...
        Rejection sends this video back to Content/Brand review for Agency to
        fix. Review timestamp comments below, then flag issues or rely on the
        overall summary. Each flagged item needs a comment.
      </>
    ) : (
      <>
        <span className="font-medium text-foreground">{videoLabel}</span> —
        Review the timestamp comments below before sending. You can reject using
        those only, or add an overall summary. Flag the video file, title,
        description, or tags, or reject thumbnails — each flagged item needs a
        comment. Thumbnail reviews are saved first, then the rejection is sent.
      </>
    )
            ...
            <label className="flex cursor-pointer gap-3 rounded-md border border-transparent p-2 hover:bg-muted/40 has-checked:border-border has-checked:bg-background">
              ...
              <div className="min-w-0 flex-1 space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  Flag issues with the rendered file (picture, sound, pacing,
                  etc.).
                </p>
                {draft.video.flag ? (
                  <Textarea
          ...
          {(asset.thumbnails?.length ?? 0) > 0 ? (
            <div className="space-y-3">
              ...
              <p className="text-xs text-muted-foreground">
                {variant === "brand" ? (
                  <>
                    Check thumbnails to include in this rejection and add a
                    comment for each selected image. Unchecked thumbnails are
                    marked approved when you submit.
                  </>
                ) : (
                  <>
                    Check thumbnails to include in this rejection and add a
                    comment for each selected image.
                  </>
                )}
              </p>
